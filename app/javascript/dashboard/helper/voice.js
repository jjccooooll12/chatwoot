...
function extractCallData(message) {
  ...
  return {
    ...
    // Snapshotted like `caller` below: the conversation can drop out of the
    // Vuex store while a call is live, so the card can't look this up later.
    ticketNumber: message?.conversation?.ticket_number,
